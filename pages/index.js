...
export default function Home() {
  ...
  return (
    <div className="bg-[#0b0e11] min-h-screen text-white font-sans pb-20">
      ...
      {/* VIDEO PREVIEW SEM NENHUM DESFOQUE NO CÓDIGO */}
      ...
      {/* PLANOS - CADA VALOR É O BOTÃO DE PAGAMENTO */}
      ...
      {/* INSTRUÇÃO ABAIXO DOS VALORES */}
      <div className="mt-8 text-center px-10">
        <p className="text-[#ff5a00] text-[10px] font-black uppercase tracking-[0.2em] mb-2 animate-pulse">
      ...
      {pixData && (
        <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center p-6 backdrop-blur-md">
          <div className="bg-[#161b22] w-full max-w-sm p-8 rounded-[3rem] border border-gray-700 text-center">
             ...
             <button onClick={() => setPixData(null)} className="text-[10px] text-gray-500 uppercase font-bold underline">Voltar e mudar plano</button>
          </div>
        </div>
      )}
      ...
      {loading && (
        <div className="fixed inset-0 bg-black/80 z-[110] flex flex-col items-center justify-center">
          ...
          <div className="text-[#ff5a00] font-black text-xs tracking-widest uppercase">Gerando Pix...</div>
        </div>
      )}
    </div>
  );
